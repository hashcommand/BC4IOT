...
QString Blockchain::getSignature (const QString& from, const QString& data)
{
    ...
    QJsonRpcMessage request2 = QJsonRpcMessage::createRequest("eth_coinbase", QJsonArray::fromVariantList(params1)) ;
                                  ...
                                  )
{
    ...
    if (response.type() == QJsonRpcMessage::Error) {
        qDebug() << "Error:Blockchain::CallFunction," << eth_methodName << response.errorMessage() ;
