...
QString Blockchain::getSignature (const QString& from, const QString& data)
{
    QVariantList    params2             ;
    QJsonArray myparam;
    QVariantMap     resultObject2       ;

    resultObject2.insert("data", data)  ;
    resultObject2.insert("from", from)  ;
    myparam.append(from);
    myparam.append(data);

    params2.append(resultObject2) ;

    QJsonRpcMessage request2 = QJsonRpcMessage::createRequest("eth_sign", myparam) ;
    QJsonRpcMessage response = client->sendMessageBlocking(request2) ;
...
QString Blockchain::getGasEstimate (const QString& from,
                                    const QString& to,
                                    const QString& data
                                    )
{
    QVariantList    params1       ;
    QVariantMap     resultObject1 ;

    resultObject1.insert("from", from) ;
    resultObject1.insert("to",   to  ) ;
    resultObject1.insert("data", data) ;

     params1.append(resultObject1) ;
     QJsonRpcMessage request1 = QJsonRpcMessage::createRequest("eth_estimateGas", QJsonArray::fromVariantList(params1));
     QJsonRpcMessage response1 = client->sendMessageBlocking(request1) ;

     return response1.result().toString() ;
}
                                  ...
                                  )
{
    ...
    resultObject.insert("gasEstimate", GAS) ;
    ...
    if (response.type() == QJsonRpcMessage::Error) {
        qDebug() << "Error:Blockchain::CallFunction," << eth_methodName << response.errorCode() ;
    }
