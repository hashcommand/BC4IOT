...
contract BC4IOT {
	...
	// Device Registration
    function addEID (uint8 _EID, uint8 _SID, uint256 hash, bytes2 signature) public 
    {
        ...
        {
            if (token_Verify (bytes32 (hash), signature) !=  systemID[_SID]) 
            {
	...
	// token_verification
	function token_Verify (bytes32 hash, bytes2 signature) public pure returns (address)
    {
        bytes32 r;
        bytes32 s;
        uint8 v;
