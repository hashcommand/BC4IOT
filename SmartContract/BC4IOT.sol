...
contract BC4IOT {
	...
	// Device Registration
    function BC4IOT_addEID (uint8 _EID, uint8 _SID, bytes32 hash, bytes2 signature) public 
    {
        ...
        {
            if (token_Verify (hash, signature, _SID) !=  true) {
	...
	// token_verification
	function token_Verify (bytes32 hash, bytes2 signature, uint8 SID) public view returns (bool)
    {
        bytes32 r;
        bytes32 s;
        uint8 v;
        ...
        if (v != 27 && v != 28) {
            ...
            return false;
                ...
                if (ecrecover(prefixedHash, v, r, s) != systemID[SID])
                return false;
                else
                return true;
    ...
    function hashCalculate (uint8 SID) public pure returns (bytes32)
    {
        
        return keccak256(BC4IOT_bytesConvert(SID));
    }
